...
class List extends Component {
   ...
   */
  cratePoly() {
    const {milkArea, otherMilkArea} = this.state;
    this.map.plugin(['AMap.MouseTool'], () => {
      ...
      polyLine.on('draw', (e) => {
        ...
        let isCross = [];
        [...milkArea, ...otherMilkArea].map((item) => {
          isCross.push({name: this.isCross(item, path)});
        });
        const show = isCross.filter((item) => {
          return item.name === true;
        });
        if (show.length > 0) {
          message.error('存在重叠区域');
          polyLine.close(true);
          return;
        } else {
          this.setState({
   ...
   */
  isCross(one, two) {
    const cross = window.AMap.GeometryUtil.ringRingClip(one, two);
    if (cross.length !== 0) {
      return true;
    } else {
      return false;
